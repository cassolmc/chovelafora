...
<script>
(function(){
  var sp=document.getElementById('splash'),cv=document.getElementById('canvas'),fim=false;
  function sair(){if(fim)return;fim=true;sp.classList.add('out');
    setTimeout(function(){sp.remove()},700);}
  new MutationObserver(function(){if((cv.width|0)>64)setTimeout(sair,400);})
    .observe(cv,{attributes:true,attributeFilter:['width','height']});
  if((cv.width|0)>64)sair();
})();
</script>
"""
